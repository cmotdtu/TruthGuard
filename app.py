...
#             sentences = [sentence.strip() for sentence in list_content]
#             similarity = calculate_similarity_percentage(query, x[0])
#             simi = round(similarity)
#             linkss.append(f'<tr> <td>{i+1}</td> <td>{link_domain}</td><td><a href="{link}">{title}</a></td><td>{"<br>".join(f"{index + 1}. {sentence}" for index, sentence in enumerate(sentences))}</td><td>{simi}</td> </tr>')
#             time.sleep(10)
